...
unet_model = unet(params['dim_x'], params['dim_y'], params['dim_z'], 1, nCommon_multiple, old_classes, alpha_relu)

# load pretrained model
print("Loading pre-trained", lobe_pretrained, 'lobe model')
unet_model.load_weights(path_pretrained_network)

model = unet_model
layer_names = [layer.name for layer in model.layers]

for m in np.arange(27):  # until leaky ReLu before after last conv layer
    model.layers.pop()
frozen_layer_names = [layer.name for layer in model.layers]
# print(frozen_layer_names)
...
for l, layer in enumerate(fm.layers):
    if l < 43:  # layer.name not in frozen_layer_names:
...
# print(df)
#
# x = fm.layers[66].output
# last_conv = Conv3D(n_classes, (1, 1, 1), activation=None, use_bias=False)(x)
# last_BN = BatchNormalization(epsilon=0.001, weights=None, momentum=0.9)(last_conv)
# last_softmax = Softmax()(last_BN)

# inputs = Input((params['dim_x'], params['dim_y'], params['dim_z'], 1))

# model1 = Model(input=inputs, output=last_softmax)
...
#
# half_frozen_model = Sequential()
# for l in np.arange(67):
#     half_frozen_model.add(fm.layers[l])
# half_frozen_model.add(last_conv)
# half_frozen_model.add(last_BN)
# half_frozen_model.add(last_softmax)

# layers = [(layer, layer.name, layer.trainable) for layer in half_frozen_model.layers]
# df = pd.DataFrame(layers, columns=['Layer Type', 'Layer Name', 'Layer Trainable'])
# print(df)
...
half_frozen_model.compile(optimizer=opt, loss=[dice_loss], metrics=[dice_coef])

half_frozen_model.summary()
...
history = half_frozen_model.fit_generator(generator=training_generator,
